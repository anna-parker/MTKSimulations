...
function run_one_sim(no_lineages::Int, rec_rate::Float64, strict, simtype, res, k_range, rounds, final_no_resolve, pre_resolve)
    ...
    for no_trees in k_range
        ...
        i_MCCs = MTK.get_infered_MCC_pairs!(i_trees, TreeKnit.OptArgs(;nMCMC=250, consistent = false, parallel=false, strict, rounds, final_no_resolve, pre_resolve))
        loc = sample(1:no_trees, 2, replace = false)
        names = [t.label for t in true_trees[rand_order][loc]]

        percentage_incompatibilities = topological_incompatibilities(true_trees[rand_order][loc][1], true_trees[rand_order][loc][2], TreeKnit.get(i_MCCs, names...))
        percentage_unresolvable_incompatibilities = unresolveable_topological_incompatibilities(true_trees[rand_order][loc][1], true_trees[rand_order][loc][2], TreeKnit.get(i_MCCs, names...))
        results[no_trees] = (percentage_incompatibilities, percentage_unresolvable_incompatibilities)
    end
...
function run_MCC_accuracy_simulations(no_sim::Int, no_lineages::Int, rec_rate::Float64; strict=true, simtype=:flu, res=0.3, k_range=2:8, rounds=2, final_no_resolve=false, pre_resolve=false)
    ...
    for no_trees in k_range
        for (p,v_dict) in zip([percentage_incompatibilities, percentage_unresolvable_incompatibilities], [percentage_incompatibilities_dict, percentage_unresolvable_incompatibilities_dict])
    ...
    return  (percentage_incompatibilities_dict, percentage_unresolvable_incompatibilities_dict)
end
...
function main()
    ...
    incomp, unres_incomp = run_MCC_accuracy_simulations(num_sim, n, r; strict, simtype, res, k_range, rounds, final_no_resolve, pre_resolve)
